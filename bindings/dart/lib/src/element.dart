...
class Element {
  final Pointer<element_t1> _ptr;
  bool _isDisposed = false;

  /// Creates a new empty element.
  Element() : _ptr = bindings.element_create() {
    if (_ptr == nullptr) {
  ...
  void setInteger(String name, int value) {
    ...
    try {
      final error = bindings.element_set_integer(_ptr, namePtr.cast(), value);
      if (error != 0) {
  ...
  void setFloat(String name, double value) {
    ...
    final namePtr = name.toNativeUtf8();
    try {
      final error = bindings.element_set_float(_ptr, namePtr.cast(), value);
      if (error != 0) {
  ...
  void setString(String name, String value) {
    ...
    try {
      final error = bindings.element_set_string(
        _ptr,
        namePtr.cast(),
        valuePtr.cast(),
      );
  ...
  void setNull(String name) {
    ...
    final namePtr = name.toNativeUtf8();
    try {
      final error = bindings.element_set_null(_ptr, namePtr.cast());
      if (error != 0) {
  ...
  void setArrayInteger(String name, List<int> values) {
    ...
    try {
      ...
      final error = bindings.element_set_array_integer(
        _ptr,
        namePtr.cast(),
        arrayPtr,
        values.length,
  ...
  void setArrayFloat(String name, List<double> values) {
    ...
    try {
      ...
      final error = bindings.element_set_array_float(
        _ptr,
        namePtr.cast(),
        arrayPtr,
        values.length,
  ...
  void setArrayString(String name, List<String> values) {
    ...
    try {
      ...
      final error = bindings.element_set_array_string(
        _ptr,
        namePtr.cast(),
        arrayPtr,
        values.length,
  ...
  void clear() {
    _ensureNotDisposed();
    bindings.element_clear(_ptr);
  }

  /// Frees the native memory associated with this element.
  void dispose() {
    if (_isDisposed) return;
    bindings.element_destroy(_ptr);
    _isDisposed = true;
  }
}
